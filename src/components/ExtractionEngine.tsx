...
export function ExtractionEngine() {
  ...
  return (
    <div className="w-full flex flex-col font-sans bg-[#050a0f] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden min-h-[700px]">
      
      {/* Top Telemetry Bar */}
      <div className="flex items-center justify-between px-6 py-3 bg-[#0a1017] border-b border-slate-800 shrink-0">
        <div className="flex items-center gap-3">
          <Network className="text-brand-jade" size={18} />
          <span className="text-slate-300 font-bold tracking-widest uppercase text-xs">APS Rules Extraction Core</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-64 bg-[#0a1017] border-r border-slate-800 p-4 flex flex-col gap-4 shrink-0 overflow-y-auto">
          <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Available Pipelines</div>
          {LOGIC_PIPELINES.map((pipeline) => (
            <button
              key={pipeline.id}
              onClick={() => setActivePipeline(pipeline)}
              className={`p-3 rounded-lg border text-left transition-all relative overflow-hidden group ${activePipeline.id === pipeline.id ? 'bg-[#0f1722] border-brand-jade/50 shadow-[0_0_15px_rgba(0,183,168,0.1)]' : 'bg-transparent border-slate-800 hover:border-slate-700'}`}
            >
              <div className="flex items-center gap-2 mb-1.5">
                <Database size={14} className={activePipeline.id === pipeline.id ? 'text-brand-jade' : 'text-slate-500'} />
                <span className={`text-xs font-bold ${activePipeline.id === pipeline.id ? 'text-slate-200' : 'text-slate-400'}`}>{pipeline.label}</span>
              </div>
        ...
        <div className="flex-1 flex flex-col relative bg-[#050a0f] overflow-hidden">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-8">
            {!executionNodes && !isProcessing && (
              <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto opacity-60">
                <GitBranch size={48} className="text-slate-700 mb-6" />
                <h3 className="text-lg font-bold text-slate-300 mb-2">Target Pipeline Selected</h3>
                <p className="text-sm text-slate-500 mb-8 leading-relaxed">Enter a system query below to extract executable logic gates and trace statutory compliance nodes.</p>
            ...
            {executionNodes && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex items-start gap-4">
                  <div className="w-8 h-8 rounded bg-slate-800 flex items-center justify-center shrink-0 mt-1 border border-slate-700">
                    <Code2 size={14} className="text-slate-300" />
                  </div>
                  <div className="flex-1">
                    <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mb-1">Execution Query</div>
                    <div className="bg-[#0a1017] p-4 rounded-xl border border-slate-800 text-slate-200 font-mono text-sm shadow-inner">
                ...
                {/* Connection Line */}
                <div className="w-px h-8 bg-slate-800 ml-4 relative">
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-jade/20 text-brand-jade text-[8px] font-mono px-2 py-0.5 rounded border border-brand-jade/30">
                    TRANSLATING
                  </div>
                </div>
                ...
                {/* Output Nodes */}
                <div className="flex items-start gap-4">
                  <div className="w-8 h-8 rounded bg-brand-jade/10 flex items-center justify-center shrink-0 mt-1 border border-brand-jade/30">
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Compiled Logic Gates</div>
                    ...
                    {executionNodes.extractedGates.length > 0 ? (
                      <div className="grid gap-3">
                        {executionNodes.extractedGates.map((gate, i) => (
                          <div key={i} className="bg-[#0a1017] border border-slate-700 rounded-xl p-4 shadow-lg hover:border-slate-500 transition-colors group">
                            <div className="flex items-center justify-between mb-3">
                              <span className="font-mono text-xs text-brand-jade font-bold">{gate.field.toUpperCase()}</span>
                              <span className="text-[10px] font-mono px-2 py-1 bg-slate-800 text-slate-300 rounded border border-slate-700">
                                Sufficiency: {gate.statutorySufficiency.toFixed(2)}
                              </span>
                            </div>
                            <div className="text-slate-200 font-mono text-sm mb-4 pl-3 border-l-2 border-slate-700">
                              {gate.value}
                            </div>
                            <div className="flex items-start gap-2 pt-3 border-t border-slate-800">
                              ...
                              <span className="text-xs text-slate-400 font-sans leading-relaxed">{gate.complianceNote}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="bg-[#0a1017] p-5 rounded-xl border border-slate-800 text-slate-400 font-mono text-xs whitespace-pre-wrap">
          ...
          {/* Bottom Command Bar */}
          <div className="p-4 bg-[#0a1017] border-t border-slate-800 shrink-0">
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full bg-[#050a0f] border border-slate-700 rounded-lg py-3 pl-8 pr-12 text-slate-200 font-mono text-sm focus:outline-none focus:border-brand-jade focus:ring-1 focus:ring-brand-jade/50 transition-all placeholder:text-slate-600"
              />
              <button 
                onClick={() => handleExecute()} 
                disabled={!input.trim() || isProcessing}
                className="absolute right-2 p-1.5 rounded-md text-slate-400 hover:text-brand-jade hover:bg-brand-jade/10 transition-colors disabled:opacity-50"
              >
