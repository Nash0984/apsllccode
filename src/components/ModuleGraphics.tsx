...
import { Database, Zap, Globe, Binary } from 'lucide-react';
        ...
        return (
          <UXFrame title="gov.systems/admin/jurisdiction" subtitle="Data Sovereignty Matrix">
            <div className="flex h-full">
              <div className="w-12 border-r border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 flex flex-col items-center py-4 gap-4">
                <div className="w-6 h-6 rounded-lg bg-brand-jade shadow-lg shadow-brand-jade/20 flex items-center justify-center">
                  <div className="w-3 h-3 border-2 border-white rounded-sm" />
                </div>
                {[0, 1, 2, 3].map(i => <div key={i} className="w-6 h-6 rounded-lg bg-slate-200 dark:bg-slate-800" />)}
              </div>
              <div className="flex-1 p-6 relative">
                <div className="grid grid-cols-3 gap-4 mb-6">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="h-16 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/30 p-3">
                      <div className="w-8 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full mb-2" />
                      <div className="w-12 h-3 bg-brand-jade/20 rounded" />
                    </div>
                  ))}
                </div>
                <svg viewBox="0 0 200 100" className="w-full text-brand-jade">
                  <rect x="10" y="10" width="180" height="80" rx="8" fill="none" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" />
                  <motion.path d="M30 30 H170 V70 H30 Z" fill="currentColor" fillOpacity="0.05" stroke="currentColor" strokeWidth="1" />
                  <motion.circle cx="100" cy="50" r="20" fill="none" stroke="currentColor" strokeWidth="2" />
                  <g transform="translate(150, 20)">
                    <rect width="35" height="12" rx="4" fill="currentColor" />
                    <text y="8" x="4" fill="white" className="text-[6px] font-bold tracking-widest">ISOLATED</text>
                  </g>
                </svg>
              </div>
            </div>
